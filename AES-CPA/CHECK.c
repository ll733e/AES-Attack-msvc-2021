...
void CPA()
{
    ...
    for (int i = 0; i < 16; i++)  OR[i] = PT[0][i];
...
void CPA_CAL(u8 OR[], u8 MK[])
{
    ...
    AES_ENC_Optimization(OR, W, CT_GEN, keysize);
    ...
    for (int i = 0; i < 16; i++)
    {
        ...
        else
        {
            ...
            printf("오류! CPA로 찾은 키가 실제 마스터키와 다릅니다.\n\n\n");
            return 0;
        }
    }
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12);
    printf("\nCPA로 찾은 키와 실제 마스터키가 같습니다.\n\n");
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 15);
    printf("%s 파일의 암호화 값 : ", ctFN);
    for (int i = 0; i < 16; i++) printf("%02X ", CT_GEN[i]);
    printf("\n마스터키로 암호화 한 값 : ");
