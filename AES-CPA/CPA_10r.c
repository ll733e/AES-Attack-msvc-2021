...
int main()
{
	...
	for (int i = 0; i < 16; i++)
	{
		...
		for (key = 0; key < 256; key++) {
			...
			for (j = 0; j < TraceNum; j++) { // hw 구하는 곳   공격지점입니다.

				u8 temp;
				CT[j][i] = SBOX[CT[j][i]];
				temp = CT[i][1]; CT[i][1] = CT[i][5]; CT[i][5] = CT[i][9]; CT[i][9] = CT[i][13]; CT[i][13] = temp;
				temp = CT[i][2]; CT[i][2] = CT[i][10]; CT[i][10] = temp; temp = CT[i][6]; CT[i][6] = CT[i][14]; CT[i][14] = temp;
				temp = CT[i][15]; CT[i][15] = CT[i][11]; CT[i][11] = CT[i][7]; CT[i][7] = CT[i][3]; CT[i][3] = temp;
